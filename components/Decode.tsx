...
const Decode = () => {
  ...
  const decodeImageMessage = (imageData: string, key: string) => {
    return new Promise<{ text: string; embeddedImage: string | null; error?: string; width: number; height: number }>((resolve, reject) => {
  ...
  const handleSubmit = async () => {
    ...
    const invalidKeyIndex = images.findIndex(img => 
      !img.key || img.key.length !== KEY_LENGTH
    );
    if (invalidKeyIndex !== -1) {
      setConfirmation(`⚠️ Encryption key for Image ${invalidKeyIndex + 1} must be exactly ${KEY_LENGTH} characters.`);
      return;
    }
    try {
      const decodedResults = await Promise.all(
        images.map(async (image) => {
          const { text, embeddedImage, error, width, height } = await decodeImageMessage(image.data, image.key);
          return { 
            text, 
            image: embeddedImage, 
            name: image.name, 
      ...
      setTimeout(() => {
        setDecodedData(prev =>
          prev.filter(data => data.error !== "Wrong key")
        );
      }, 2000);
      setTimeout(() => setConfirmation(""), 2000);
    } catch (error) {
  ...
  const isDecodingSuccessful = decodedData.length > 0 && 
    decodedData.every(data => 
      !data.error && 
      (data.text !== "No hidden data found" || data.image)
    );
        ...
        >
              ...
              {images.map((image, index) => (
                <div key={index} className="mb-4">
                  <Input
                    ...
                    variant="bordered"
                    color={image.key && image.key.length !== KEY_LENGTH && decodedData[index]?.error === "Wrong key" ? "danger" : "primary"}
                    classNames={{
                  ...
                  />
                  {image.key && image.key.length !== KEY_LENGTH && (
          ...
          {decodedData.length > 0 ? (
            <>
              ...
              >
                {isDecodingSuccessful ? "Decode More Images" : "Submit Again"}
              </Button>
            </>
          ) : (
            <Button
